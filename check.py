...
def calc_std_times_updated(row, indices):
    try:
        ...
        suffix_map = {
            ...
            "Class 5": "C5", "Group": "GR", "Griffin": "G", "4 Year Olds": "C2"
        }
...
def _compute_style(r):
    """
    Compute Style (E, E/P, P, S) based on PIR_Sec average.
    """
    try:
        dist = int(r.get("Distance", 0))
        if dist not in [1000, 1200, 1400, 1600, 1650, 1800, 2000, 2200, 2400]:
            return None

        num_secs_map = {1000: 3, 1200: 3, 1400: 4, 1600: 4, 1650: 4, 1800: 5, 2000: 5, 2200: 6, 2400: 6}
        avg_secs_map = {1000: 2, 1200: 2, 1400: 3, 1600: 3, 1650: 3, 1800: 4, 2000: 4, 2200: 5, 2400: 5}

        num_secs = num_secs_map.get(dist)
        avg_secs = avg_secs_map.get(dist)
        if not num_secs or not avg_secs:
            return None

        pir_sum = sum(int(r.get(f"PIR_Sec_{i}", 0) or 0) for i in range(1, num_secs + 1))
        styler = round(pir_sum / avg_secs, 0)
...
calc_funcs = {
    "B400": lambda r: calc_section_times(r, {1000: 2, 1200: 2, 1400: 3, 1600: 3, 1650: 3, 1800: 4, 2000: 4, 2200: 5, 2400: 5}, ["Sec_1_Time", "Sec_2_Time", "Sec_3_Time", "Sec_4_Time", "Sec_5_Time"]),
    "STDB400": lambda r: calc_std_times_updated(r, [0, 1]) if r["Distance"] in [1000, 1200] else calc_std_times_updated(r, [0, 1, 2]) if r["Distance"] in [1400, 1600, 1650] else calc_std_times_updated(r, [0, 1, 2, 3]) if r["Distance"] in [1800, 2000] else calc_std_times_updated(r, [0, 1, 2, 3, 4]) if r["Distance"] in [2200, 2400] else None,
    "TO": lambda r: calc_section_times(r, {1000: 3, 1200: 3, 1400: 4, 1600: 4, 1650: 4, 1800: 5, 2000: 5, 2200: 6, 2400: 6}, ["Sec_1_Time", "Sec_2_Time", "Sec_3_Time", "Sec_4_Time", "Sec_5_Time", "Sec_6_Time"]),
    "STDTO": lambda r: calc_std_times_updated(r, [0, 1, 2]) if r["Distance"] in [1000, 1200] else calc_std_times_updated(r, [0, 1, 2, 3]) if r["Distance"] in [1400, 1600, 1650] else calc_std_times_updated(r, [0, 1, 2, 3, 4]) if r["Distance"] in [1800, 2000] else calc_std_times_updated(r, [0, 1, 2, 3, 4, 5]) if r["Distance"] in [2200, 2400] else None,
    "L400": lambda r: float(r.get({1000: "Sec_3_Time", 1200: "Sec_3_Time", 1400: "Sec_4_Time", 1600: "Sec_4_Time", 1650: "Sec_4_Time", 1800: "Sec_5_Time", 2000: "Sec_5_Time", 2200: "Sec_6_Time", 2400: "Sec_6_Time"}.get(r["Distance"]), 0) or 0),
    "STDL400": lambda r: calc_std_times_updated(r, 2) if r["Distance"] in [1000, 1200] else calc_std_times_updated(r, 3) if r["Distance"] in [1400, 1600, 1650] else calc_std_times_updated(r, 4) if r["Distance"] in [1800, 2000] else calc_std_times_updated(r, 5) if r["Distance"] in [2200, 2400] else None,
    "L800": lambda r: calc_section_times(r, {1000: 2, 1200: 2, 1400: 2, 1600: 2, 1650: 2, 1800: 2, 2000: 2, 2200: 2, 2400: 2}, ["Sec_2_Time", "Sec_3_Time", "Sec_3_Time", "Sec_4_Time", "Sec_4_Time", "Sec_5_Time", "Sec_5_Time", "Sec_6_Time"]),
    "STDL800": lambda r: calc_std_times_updated(r, [1, 2]) if r["Distance"] in [1000, 1200] else calc_std_times_updated(r, [2, 3]) if r["Distance"] in [1400, 1600, 1650] else calc_std_times_updated(r, [3, 4]) if r["Distance"] in [1800, 2000] else calc_std_times_updated(r, [4, 5]) if r["Distance"] in [2200, 2400] else None,
    ...
    "Style": _compute_style  # Fixed reference
}
...
database_name = "HKData" + input("Please input the database file name (e.g. HKData20250303)(Date only): ").strip() + ".csv"
